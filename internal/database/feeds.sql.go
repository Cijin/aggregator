...
type ListFeedsToFetchParams struct {
	Limit  int32
	Offset int32
}

func (q *Queries) ListFeedsToFetch(ctx context.Context, arg ListFeedsToFetchParams) ([]Feed, error) {
	rows, err := q.db.QueryContext(ctx, listFeedsToFetch, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
